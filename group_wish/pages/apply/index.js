...
//获取应用实例
const app = getApp();
import {getUserProfile, teamApply} from '../../api/member';
...
Page({
    ...
    loadUserInfo(){
      ...
      getUserProfile({}, true)
        .then(function(res) {
          ...
          if (res.audit && res.audit.status !== 1) {
            that.setData({
              urlList: res.audit.urls,
              auditData: res.audit,
            })
          }
        })
    ...
    forTeamApply: function(params) {
      let that = this;
      teamApply(params).then(function(res) {
    ...
    onShareAppMessage() {
      let {curId} = this.data;
      return {
        path: 'pages/apply/index?page=apply&id='+curId
      }
    },
